...
d3.csv("./data/pokemon_alopez247.csv").then(rawData =>{ 
    ...
    //Plot 1: Bar Chart for the Generation
    //The bar chart is able to show the amount of each Pokemon per Generation, it gives the idea of how much there are for each one, along with easy comparision between the amounts

    //Processing data and amount for Generations
    const primaryTypeCounts = processedData.reduce((s, { Generation }) => (s[Generation] = (s[Generation] || 0) + 1, s), {});
    const primaryTypeData = Object.keys(primaryTypeCounts).map((key) => ({ Generation: key, count: primaryTypeCounts[key] }));
    ...
    const x2 = d3.scaleBand()
        .domain(primaryTypeData.map(d => d.Generation))
        .range([0, barWidth])
        .paddingInner(0.3)
        .paddingOuter(0.2);
    ...
    const parrallelCoordinates = svg.append("g")
    ...
    // Highlight the Generation that is Hovered
    var highlight = function(d){
        ...
        // Second: The hovered Generation takes its color
        d3.selectAll(".Gen" + selected)
            .transition().duration(200)
    ...
    //Draw Lines
    parrallelCoordinates
        .selectAll("myPath")
        .data(processedData)
        .enter().append("path")
        .attr("class", function (d) { return "line " + "Gen" + d.Generation } ) // 2 class for each line: 'line' and the group name
        .attr("d",  path)
        .style("fill", "none")
        .style("stroke", function(d){ return( color(d.Generation))}) 
        .style("opacity", 0.25)
    ...
    //Draw Axises
    parrallelCoordinates.selectAll("myAxis")
    ...
    //Title
    svg.append("text").attr("x", titleLeft).attr("y", titleTop).text("Do Pokemon Attributes Change Across Generations?").style("font-size", "35px").attr("alignment-baseline","middle").attr("text-anchor", "middle")

    //Legend
    svg.append("text").attr("x", width - 60).attr("y", titleTop + 50).text("Legend").style("font-size", "15px").attr("alignment-baseline","middle")
    for (i in generations) {
        svg.append("circle").attr("cx", width - 55).attr("cy", titleTop + 75 + 20*i).attr("r", 6).style("fill", colorScheme[i])
        svg.append("text").attr("x", width - 45).attr("y", titleTop + 75 + 20*i).text("Gen " + generations[i]).style("font-size", "12px").attr("alignment-baseline","middle")
    }
    ...
    //Code Fragments for Filterable Parallel Coordinates: https://d3-graph-gallery.com/graph/parallel_custom.html
});
